...
Для поднятия сервера нужно запустить скрипт server.py. В консоле выведется локальный IP-адрес. Этот адрес нужно скопировать в глобальную переменную SERVER_IP в скрипте клиента client.py. Также файл accounts.db должен находится в одной папке с server.py, чтобы сервер мог использовать уже сохраненные аккаунты. Чтобы клиенты могли отправлять ссобщения на сервер, его нужно сначала запустить, выполнив server.py, после этого можно подключать клиентов выполняя скрипт client.py. Если работа скрипта server.py завершилась, то для отправки клиентами сообщений нужно заново запустить скрипт server.py и перезапустить клиентов, выполняя скрипт client.py.
...
- database. Создать новую базу данных для регистрации новых пользователей или выбрать уже существующую
- exit. Завершить работу сервера
- help. Вывести справочную информацию

Графический интерфейс клиента состоит из окон регистрации и самого чата. В окне регистрации нужно указать имя пользователя и пароль, также там находятся кнопки входа и регистрации нового пользователя. Окно чата состоит из текстового поля, поля ввода текста и кнопки Send. Комманды сервера вводятся в консоле, после запуска server.py.
...
Чтоюбы различать одно сообщение от другого используется хэш-сумма md5. Текст сообщения представляет собой строку, зашифрованную методом шифрования Fernet из модуля cryptography. В зависимости от типа сообщения текст_сообщения может отсутсвовать, что означает выполнить соответствующие действия в зависимости от значения хеш суммы.
...
Чтобы создать новую базу данных нужно запустить скрипт сервера server.py, в консоле ввести database, после чего с новой строки ввести имя новой базы данных, которое должно заканчиваться на .db. После этого клиенты могут регистрироваться в новой базе данных с помощью кнопки Sign up.
